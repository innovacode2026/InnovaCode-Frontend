...
import { getDireccion, calcularEnvio } from './Profile'

export default function PaginaCarrito(ctx: AppContext) {
  const { role, user, carrito, products, navigate, updateCartQty, removeFromCart, crearPedido } = ctx
  const [busy, setBusy] = useState<string | null>(null)
  const [creando, setCreando] = useState(false)
  const [mensaje, setMensaje] = useState<{ tipo: 'ok' | 'error'; texto: string } | null>(null)

  const direccion = user ? getDireccion(user.id) : null
  const costoEnvio = direccion ? calcularEnvio(direccion.ciudad, carrito.total) : null
  const totalConEnvio = carrito.total + (costoEnvio ?? 0)
        ...
        <div className="bg-surface border-b border-border">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display font-700 text-2xl text-white">Carrito de compras</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {carrito.items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="font-display font-700 text-white text-xl mb-2">Tu carrito está vacío</h3>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-3">
                {carrito.items.map(item => (
                  <div key={item.productoId} className="bg-surface rounded-2xl border border-border p-4 flex gap-4">
                    <div
                      onClick={() => navigate('product', item.productoId)}
                      className="w-24 h-20 sm:w-28 sm:h-22 rounded-xl overflow-hidden bg-white/5 flex-shrink-0 cursor-pointer"
                    >
                      <img
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3">
                        <button
                          onClick={() => navigate('product', item.productoId)}
                          className="font-display font-600 text-white text-sm leading-tight line-clamp-2 text-left hover:text-primary transition-colors cursor-pointer"
                        >
                          {item.nombre}
                        </button>
                        <button
                          ...
                          className="flex-shrink-0 p-2 rounded-lg text-gray-400 hover:text-danger hover:bg-danger/10 transition-colors disabled:opacity-40 cursor-pointer"
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <div className="flex items-center border border-border rounded-xl">
                          <button
                            ...
                            className="w-9 h-9 flex items-center justify-center text-gray-400 hover:bg-white/5 disabled:opacity-30 cursor-pointer"
                          >
                            <MinusIcon size={13} />
                          </button>
                          <span className="w-9 text-center font-semibold text-white text-sm">{item.cantidad}</span>
                          <button
                            onClick={() => cambiarCantidad(item.productoId, item.cantidad + 1)}
              ...
              <div>
                <div className="bg-surface rounded-2xl border border-border p-5 sticky top-20">
                  <h3 className="font-display font-700 text-white text-lg mb-4">Resumen del pedido</h3>
                  <div className="space-y-2 text-sm mb-4">
                    ...
                    <div className="flex justify-between text-gray-400">
                      ...
                      {costoEnvio === null ? (
                        ...
                        <span className="text-white">${costoEnvio.toLocaleString('es-CO')}</span>
                      )}
                    </div>
                    {direccion && (
                      <div className="flex justify-between items-start text-gray-500 text-xs">
                        <span>Entregar en</span>
                        <span className="text-right max-w-[55%] leading-tight">
                          {direccion.direccion}, {direccion.barrio}<br />{direccion.ciudad}
                        </span>
                      </div>
                    )}
                    <div className="border-t border-border pt-3 flex justify-between font-display font-700 text-white text-base">
                      <span>Total</span>
                      <span>${totalConEnvio.toLocaleString('es-CO')}</span>
                    </div>
                  </div>
                  <button
                    onClick={hacerPedido}
                  ...
                  {costoEnvio === null && role === 'CLIENTE' && (
                    <p className="text-xs text-gray-500 mt-3 text-center">
                      <button onClick={() => navigate('profile')} className="text-primary hover:underline cursor-pointer">
                        Configura tu dirección
                      </button>{' '}
                      para ver el costo de envío
                    </p>
                  )}
                  {costoEnvio === 0 && (
                    <p className="text-xs text-success mt-3 text-center">¡Envío gratis a {direccion?.ciudad}!</p>
                  )}
                </div>
              </div>
            </div>
