...
export default function DetalleProducto(ctx: AppContext) {
  ...
  const submitComment = async () => {
    ...
    setCommentError('')
    try {
      const nuevo = await crearComentario(product.id, { puntuacion: userRating, contenido: commentText.trim() })
  ...
  const renderStars = (rating: number, interactive = false, size = 18) => {
    ...
    return Array.from({ length: 5 }, (_, i) => (
      ...
      >
        <StarIcon size={size} filled={i < active} className={i < active ? 'text-amber-400' : 'text-white/20'} />
      </button>
    ))
  }
      ...
      <div className="flex-1 bg-background">
        ...
        <div className="bg-surface border-b border-border">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-2 text-sm text-gray-400">
            ...
            <ChevronRightIcon size={14} />
            <span className="text-white/70 font-medium truncate max-w-xs">{product.name}</span>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-surface rounded-2xl border border-border overflow-hidden mb-6">
            <div className="grid lg:grid-cols-2 gap-0">
              ...
              <div className="border-r border-border">
                <div className="relative aspect-[6/5] bg-white/5 overflow-hidden rounded-tl-2xl">
                  ...
                  {media.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 text-white flex items-center justify-center disabled:opacity-30 hover:bg-black/70 transition-colors cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 text-white flex items-center justify-center disabled:opacity-30 hover:bg-black/70 transition-colors cursor-pointer"
                      >
                  ...
                  <div className="flex gap-2 p-4 overflow-x-auto">
                    {media.map((item, i) => (
                      <button
                        ...
                        className={`relative w-16 h-12 rounded-lg overflow-hidden border-2 transition-all cursor-pointer shrink-0 bg-white/5 ${
                          activeMedia === i ? 'border-primary' : 'border-border hover:border-white/30'
                        }`}
                      >
                        {item.type === 'video' ? (
                          <>
              ...
              <div className="p-6 lg:p-8 flex flex-col">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <span className="text-xs text-gray-400 font-medium capitalize bg-white/5 px-2 py-0.5 rounded-full border border-border">
                      {product.category}
                    </span>
                    <span className={`ml-2 text-xs font-semibold px-2 py-0.5 rounded-full ${
                      product.status === 'active' ? 'bg-success/10 text-success' : 'bg-white/5 text-gray-400'
                    }`}>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all border cursor-pointer ${
                        isSaved
                          ...
                          : 'bg-white/5 text-gray-400 border-border hover:border-red-500/30 hover:text-red-400'
                      }`}
                    >
                ...
                <h1 className="font-display font-700 text-white text-xl sm:text-2xl leading-tight mb-2">
                  {product.name}
                </h1>
                ...
                {product.colors && product.colors.length > 0 && (
                  <div className="mb-5">
                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-2">
                      Color — <span className="text-white normal-case tracking-normal font-semibold">
                      ...
                      {product.colors.map(color => (
                        <button
                          ...
                          style={{
                            ...
                            border: selectedColor === color.hex ? '3px solid #8B5CF6' : '2px solid rgba(255,255,255,0.15)',
                            boxShadow: selectedColor === color.hex ? '0 0 0 2px rgba(13,21,38,1), 0 0 0 4px #8B5CF6' : 'none',
                            outline: 'none',
                          }}
                        />
                      ))}
                ...
                <div className="font-display font-800 text-3xl text-white mb-5">
                ...
                {role === 'CLIENTE' && sellable && (
                  <div className="mb-5">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="flex items-center border border-border rounded-xl bg-white/5">
                        <button
                          onClick={() => setCantidad(c => Math.max(1, c - 1))}
                          disabled={cantidad <= 1}
                          className="w-10 h-10 flex items-center justify-center text-gray-400 hover:bg-white/5 disabled:opacity-30 cursor-pointer"
                        >
                          <MinusIcon size={14} />
                        </button>
                        <span className="w-10 text-center font-semibold text-white">{cantidad}</span>
                        <button
                ...
                {role === 'CLIENTE' && !sellable && product.stock <= 0 && (
                  <div className="mb-5 bg-white/5 rounded-xl p-4 text-center text-sm text-gray-400">
                ...
                {role !== 'CLIENTE' && (
                  <div className="mb-5 bg-white/5 rounded-xl p-4 text-center">
                    <p className="text-sm text-gray-400 mb-3">Inicia sesión para agregar este producto a tu carrito.</p>
                    <button
                ...
                <div className="grid grid-cols-2 gap-3 text-sm mb-5 bg-white/5 rounded-xl p-4">
                  ...
                  <div>
                    ...
                    <span className="text-white font-medium font-mono text-xs">{product.sku || '—'}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="text-white font-medium">{new Date(product.createdAt).toLocaleDateString('es-CO')}</span>
                  </div>
                </div>
                ...
                {product.features.length > 0 && (
                  <div className="flex-1">
                    <h3 className="font-display font-600 text-gray-400 text-xs uppercase tracking-wider mb-2">Características principales</h3>
          ...
          <div className="bg-surface rounded-2xl border border-border overflow-hidden mb-6">
            ...
            <div className="p-6">
              {activeTab === 'description' && (
                <p className="text-gray-300 leading-relaxed">{product.description || 'Sin descripción disponible.'}</p>
              )}
              {activeTab === 'features' && (
              ...
              {activeTab === 'reviews' && (
                <div className="space-y-6">
                  <div className="space-y-4">
                    {comments.length === 0 ? (
                      <div className="bg-white/5 rounded-xl p-6 text-center text-sm text-gray-400">
                        Aún no hay reseñas para este producto. ¡Sé el primero en opinar!
                      </div>
                    ) : (
                      comments.map(comment => (
                        <div key={comment.id} className="p-4 rounded-xl border border-border bg-white/5">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold bg-white/10 text-gray-300">
                                {iniciales(comment.usuario)}
                              </div>
                              <div>
                                <div className="font-semibold text-sm text-white">{comment.usuario}</div>
                                <div className="flex items-center gap-1 mt-0.5">
                                  {Array.from({ length: 5 }, (_, i) => (
                                    <StarIcon key={i} size={11} filled={i < comment.puntuacion} className={i < comment.puntuacion ? 'text-amber-400' : 'text-white/20'} />
                                  ))}
                          ...
                          <p className="mt-3 text-sm text-gray-300 leading-relaxed">{comment.contenido}</p>
                        </div>
                      ))
                    )}
                  </div>
                  ...
                  {role === 'CLIENTE' && (
                    <div className="border-t border-border pt-5">
                      ...
                      {submitted ? (
                        <div className="bg-success/10 text-success border border-success/20 rounded-xl p-4 flex items-center gap-2 text-sm">
                          <CheckIcon size={16} />
                        ...
                        <div className="space-y-3">
                          {commentError && (
                            <div className="bg-danger/10 text-danger border border-danger/20 rounded-xl p-3 text-sm">
                              {commentError}
                            </div>
                          )}
                          <div>
                            <label className="text-xs text-gray-400 font-medium block mb-1.5">Calificación</label>
                            <div className="flex items-center gap-1">
                              {renderStars(userRating, true, 22)}
                              {userRating > 0 && <span className="text-sm text-amber-400 font-medium ml-2">{userRating}/5</span>}
                            </div>
                          </div>
                          <div>
                            <label className="text-xs text-gray-400 font-medium block mb-1.5">Tu comentario</label>
                            <textarea
                              value={commentText}
                              onChange={e => setCommentText(e.target.value)}
                              placeholder="Comparte tu experiencia con este producto..."
                              rows={4}
                              className="w-full text-sm border border-border rounded-xl p-3 focus:outline-none focus:border-primary/50 resize-none placeholder:text-gray-500 text-white bg-white/5"
                            />
                          </div>
                          <button
                  ...
                  {role === 'guest' && (
                    <div className="border-t border-border pt-5">
                      <div className="bg-white/5 rounded-xl p-4 text-center">
                        <p className="text-sm text-gray-400 mb-3">Inicia sesión para calificar y comentar este producto.</p>
          ...
          {related.length > 0 && (
            <div>
              <h2 className="font-display font-700 text-white text-lg mb-4">Productos relacionados</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {related.map(p => (
                  <div
                    key={p.id}
                    onClick={() => navigate('product', p.id)}
                    className="bg-surface rounded-xl border border-border p-3 flex gap-3 hover:border-primary/30 transition-all cursor-pointer"
                  >
                    <img src={p.image} alt={p.name} className="w-20 h-16 object-cover rounded-lg shrink-0 bg-white/5" />
                    <div className="min-w-0">
                      <div className="font-display font-600 text-sm text-white/80 line-clamp-2 leading-tight">{p.name}</div>
